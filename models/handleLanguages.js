...
exports.mergeAndTally = async function(langs, ctrys, ranked) {
    ...
    for (let country of ctrys) {
        // loop through ctrys of chosen continent
        for (let lang of langs) {
            // in loop find corresponding languages
            if (lang.countrycode === country.code) {
                if (!result[lang.language]) {
                    // if counter for lang not exists, create
                    result[lang.language] = Math.floor((lang.percentage * country.population) / 100);
                } else {
                    // if counter for lang exists, add to it
                    result[lang.language] += Math.floor((lang.percentage * country.population) / 100);
                }
            }
        }
    ...
    sortable.sort(function(a, b) {
        // sort array on values, descending
        return b[1] - a[1];
    });

    sortable.forEach(function(item) {
        // format numeric values for print
